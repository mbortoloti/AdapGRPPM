...
#         stopping_criterion=StopAfterIteration(1000)|StopWhenChangeLess(M,1.e-3),
#         sub_stopping_criterion=StopAfterIteration(1000)|StopWhenGradientNormLess(1.e-3)
#       );
...
#           stopping_criterion=StopAfterIteration(1000)|StopWhenChangeLess(M,1.e-3),
#           sub_stopping_criterion=StopAfterIteration(1000)|StopWhenGradientNormLess(1.e-38)
#        );
##################################################################################################
...
plot(mdim,T[:,3],label="Adap-RPPM",lw=2,color=:blue)#,xticks = ([0.0,1000.0,2000.0,3000.0,4000.0.0,5000.0],["0","1000","2000","3000","4000","5000"]))
xlabel!("Manifold Dimension "*L"d = " * "dim " * L"\mathbb{P}^n_{++}")
ylabel!("run time (sec.)")

#A = [mdim ones(length(mdim))]
#coef = A \ T[:,3]

#lin(x) = coef[1]*x+coef[2]

#println("$(coef[1]) x + $(coef[2])")


#plot!(mdim,lin.(mdim),label="",lw=2,color=:blue)

#function plot_triangle(p1, p2, p3)
...
#plot_triangle([2500,lin(2500)-0.5,0.0],[4000,lin(2500)-0.5,0.0],[4000,lin(4000)-0.5,0.0])

#if nguess == 1
savefig("example623.png")
#end
